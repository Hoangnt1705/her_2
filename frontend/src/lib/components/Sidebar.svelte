<script>
    ...
    function handleClick(chatId, source) {
    if(source === "ParseRecruiterChat") goto(`/parse-recruiter/${chatId}`); 
    else goto(`/resume-ai/${chatId}`);
    }
    
    async function handleEditClick(chatId, title, source) {
        editingId = chatId;
        chatTitleSideBar = title;
        typeConversation = source;
        await tick();
        focusBind.focus();
    };
    ...
    afterUpdate(async () => {
    await tick();
    activeChatId.set(currentPath.split('/')[2]);
});
    $: updateActiveChat();
    // afterUpdate(async () => {
    //     await tick();
        ...
        <div id="sidebar" class:hidden={!$sidebar} >
            <div class="float-top">
                ...
                <ul class="conversations h-[600px] max-w-xs flex flex-col mb-5 overflow-y-auto
                    ...
                    <li class="inline-flex items-center text-sm font-medium text-gray-800 my-1" in:receive={{ key: chat._id }} out:send={{ key: chat._id }} id="{chat.source}-{chat._id}">
                        {#if chat.source === "ParseRecruiterChat"}
                        <span class="w-1 h-1 inline-block bg-gray-900 rounded-full mr-1"></span>
                        {:else}
                        <span class="w-1 h-1 inline-block bg-purple-600 rounded-full mr-1"></span>
                        {/if}
                        ...
                        <input
                            ...
                            on:click={() => handleClick(chat._id, chat.source)}
                        data-sveltekit-preload-code
                        ...
                        <div class:edit-buttons={$activeChatId === chat._id} class:ac={$activeChatId !== chat._id} class="gap-1">
                            {#if editingId !== chat._id}
                            <button on:click={() => handleEditClick(chat._id, chat.title, chat.source)}>
            ...
            <div out:send>
                ...
                <div in:fade>
                    <div class="login-wrap" style="margin: 22px;">
                        <div class="mb-3">
                            <p style="color: #0f1d3b; font-weight: 600; font-size: 0.8rem;" class="font-extrabold">Login</p> <!-- Darkened the blue for better contrast -->
                            <p style="color: #5c6b87; font-weight: 500; font-size: 0.8rem;">
                              Save your chat history, share chats, and personalize your experience.
                            </p> <!-- Darkened the gray and increased font weight -->                      
                        </div>
                    <ButtonLogin />
                    </div>
                </div>
            ...
            <Modal deleteChatTitle={deleteChatTitle} chatId={deleteId} accessToken={$accessToken} source={typeConversation}/>
